// lab says to run with `npm run test` but can also use `npm test ./__tests__/lab8.test.js`
...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const addToCartBtn = await shadowRoot.$('button');
    await addToCartBtn.click();
    const innerText = await addToCartBtn.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    // for loop starts at i=1 since we already clicked the button of the 0th element in the previous step
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // source: https://stackoverflow.com/questions/55236975/how-to-reload-page-in-puppeteer
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // source: https://stackoverflow.com/questions/55725295/puppeteer-get-localstorage-from-a-website
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for(let i = 0; i<prodItems.length; i++){
      const shadowRoot = await prodItems[i].getProperty('shadowRoot');
      const removeBtn = await shadowRoot.$('button');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i<20; i++) {
      ...
      const addToCartBtn = await shadowRoot.$('button');
      const innerText = await addToCartBtn.getProperty('innerText');
      expect(innerText['_remoteObject'].value).toBe('Add to Cart');
    }
    const cartCount = await page.$('#cart-count');
    const innerText = await cartCount.getProperty('innerText');
    expect(innerText['_remoteObject'].value).toBe('0');
